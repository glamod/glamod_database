CREATE TABLE __INSERT_SCHEMA__.header_table (
  ...
  primary_station_id_scheme INT REFERENCES id_scheme(scheme),
  longitude NUMERIC,
  latitude NUMERIC,
  location_accuracy NUMERIC,
  location_method INT REFERENCES location_method(method),
  location_quality INT REFERENCES location_quality(quality),
  ...
  sea_level_datum INT REFERENCES sea_level_datum(datum),
  report_meaning_of_timestamp INT REFERENCES meaning_of_time_stamp(meaning),
  report_timestamp TIMESTAMP WITH TIME ZONE,
  report_duration INT REFERENCES duration(duration),
  report_time_accuracy NUMERIC,
  report_time_quality INT REFERENCES time_quality(quality),
  report_time_reference INT REFERENCES time_reference(reference),
  profile_id VARCHAR REFERENCES profile_configuration(profile_id),
  events_at_station INT[],
  report_quality INT  REFERENCES quality_flag(flag),
  duplicate_status INT REFERENCES duplicate_status(status),
  duplicates VARCHAR[],
  record_timestamp TIMESTAMP WITH TIME ZONE,
  history VARCHAR ,
  processing_level INT REFERENCES report_processing_level(level),
  processing_codes INT[],
  source_id VARCHAR REFERENCES source_configuration(source_id),
  source_record_id VARCHAR,
  location geography,
  PRIMARY KEY(report_id),
  FOREIGN KEY( primary_station_id, station_record_number) REFERENCES station_configuration( primary_id, record_number)
);
