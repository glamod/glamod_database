CREATE OR REPLACE FUNCTION validate_observations_table()
RETURNS trigger AS 
$BODY$
DECLARE
  table_size INT;
BEGIN
IF NOT new.processing_code <@ (select array_agg(code) from processing_code) THEN
...
IF new.code_table IS NOT NULL THEN
  SELECT count(code_table) as nrows into table_size from observation_code_table where code_table = new.code_table;
